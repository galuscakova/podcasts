...
def greedy_decode(model: PreTrainedModel,
    ...
    past = model.module.get_encoder()(input_ids, attention_mask=attention_mask)
    past = past + (None,)
    next_token_logits = None
    for _ in range(length):
        model_inputs = model.module.prepare_inputs_for_generation(
            decode_ids,
            past=past,
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        examples = (
            processor.get_test_examples(args.train_test_file, args.query_file, args.qrel_file) if evaluate 
            else processor.get_train_examples(args.train_test_file, args.query_file, args.qrel_file)
        )
